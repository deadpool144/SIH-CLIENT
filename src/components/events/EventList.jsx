...
export default function EventsPage() {
  ...
  const filteredEvents = events.filter((event) => {
    const matchFilter =
      filter === "All Events" || event.category?.toLowerCase() === filter.toLowerCase();
    ...
    <div className="bg-[#bca46a] min-h-screen font-['Segoe_UI',Arial,sans-serif]">
      ...
      <main className="max-w-6xl mx-auto py-12 px-4">
        ...
        {filteredEvents.length === 0 ? (
          <p className="text-gray-200">No events yet. Be the first to create one!</p>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredEvents.map((event) => (
              <div
                key={event._id}
                className="bg-[#23262B] rounded-lg shadow-lg overflow-hidden flex flex-col"
              >
                {event.imageUrl && (
                  <img
                    src={event.imageUrl}
                ...
                <div className="p-5 flex flex-col flex-1">
                  <span className="inline-block px-3 py-1 mb-3 rounded-md text-white text-sm font-semibold bg-[#b48a23]">
